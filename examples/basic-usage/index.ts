...
function testMatch() {
  ...
  let filePath =
    "D:/src/AdsSnR/private/De.Snr.Compass.Product/test/BackendWorker/obj/src/test/aaa.cs";
 ...
 */
async function iterateAllChromaRecords(
  ...
  try {
    ...
    try {
      ...
      if (count === 0) {
        console.log(
          `⚠️ Collection ${collectionName} has no records, skipping...`
        );
        return [];
      }
      ...
      console.log(
        `   ✅ Collection ${collectionName} processed in ${collectionTime}ms`
      );
    } catch (error) {
      console.log(
        `   ⚠️  Collection ${collectionName} not found or not accessible:`,
        error.message
      );
    }
    ...
    const totalTime = Date.now() - startTime;
    console.error(
      `❌ Error during ChromaDB iteration after ${totalTime}ms:`,
      error
    );
    throw error;
  }
...
async function testSemanticSearchHybrid(codebasePath: string, query: string) {
  let host = "localhost:19530";

  const vectorDatabase = new MilvusVectorDatabase({
    address: host,
  });

  let codeAgentEndpoint =
  ...
  let results = await context.semanticSearch(
    codebasePath,
    query,
    3,
    0.3,
