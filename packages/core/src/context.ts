...
import { getGitRepoName, matchesIgnorePattern } from "./utils/utils";
...
export class Context {
  ...
  private mergeSearchResults(
    ...
    console.log(
      `🔄 Merged ${clientResults.length} client + ${serverResults.length} server = ${mergedResults.length} results (server baseline with client replacements + ${unusedClientResults.length} unused client results)`
    );
  ...
  async semanticSearch(
    ...
    if (isHybrid === true) {
      ...
      const [searchResults, searchResultsFromServer] = await Promise.all([
        this.vectorDatabase.search(collectionName, queryEmbedding.vector, {
          topK,
          threshold,
          filterExpr,
        }),
  ...
  private async getServerRepoIgnorePattern(
    ...
    try {
      const response = await fetch(
        `${this.codeAgentEndpoint}/get_ignore_patterns?codebase=${gitRepoName}`
      );

      if (!response.ok) {
        console.error(
          `[IGNORE-PATTERN] ❌ Server request failed with status: ${response.status}`
        );
        return [];
      }
      ...
      console.log(
        `[IGNORE-PATTERN] ✅ Successfully retrieved ignore patterns for codebase: ${gitRepoName}, ignore patterns: ${serverData.ignore_patterns.join(
          ", "
        )}`
      );
      ...
      console.error(
        `[IGNORE-PATTERN] ❌ Error getting ignore patterns:`,
        error.message || error
      );
      return [];
    }
