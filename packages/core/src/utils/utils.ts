 ...
 */
export function getGitRepoName(folderPath: string): {
  gitRoot: string;
  repoName: string;
} {
  try {
    ...
    const originUrlMatch = config.match(
...
export async function getAISearchKey(
  codeAgentEndpoint: string
): Promise<string> {
  try {
    const response = await fetch(`${codeAgentEndpoint}/get_ai_search_key`);

    if (!response.ok) {
      console.error(
        `[AI-SEARCH] ❌ Failed to get AI Search key with status: ${response.status}`
      );
      return "";
    }

    const data = (await response.json()) as any;
    return data.ai_search_key || "";
  } catch (error: any) {
    console.error(
      `[AI-SEARCH] ❌ Error getting AI Search key:`,
      error.message || error
    );
    return "";
  }
...
export async function checkServerSnapshot(
  codeAgentEndpoint: string,
  codebasePath: string
): Promise<{ json: any; error: boolean; version: string }> {
  try {
    console.log(
      `[SERVER-CHECK] 🔍 Checking server snapshot for codebase: ${codebasePath}`
    );
    const response = await fetch(
      `${codeAgentEndpoint}/get_snapshot?codebase=${codebasePath}`
    );

    if (!response.ok) {
      console.error(
        `[SERVER-CHECK] ❌ Server request failed with status: ${response.status}`
      );
      return {
        json: {
          error: `Failed to connect to server (status: ${response.status}). Please ensure the server is running.`,
        },
        error: true,
        version: "",
      };
    ...
    if (serverData.error !== "success") {
      console.error(
        `[SERVER-CHECK] ❌ Code is not onboarded at server side: ${serverData.error}`
      );
      return {
    ...
    console.log(
      `[SERVER-CHECK] ✅ Server snapshot check passed for codebase: ${codebasePath}`
    );
    return {
    ...
    console.error(
      `[SERVER-CHECK] ❌ Error checking server snapshot:`,
      serverError.message || serverError
    );
    return {
      json: {
        error: `Failed to check server snapshot: ${
          serverError.message || serverError
