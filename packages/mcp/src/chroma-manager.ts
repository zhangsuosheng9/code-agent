...
export class ChromaManager {
  ...
  public async start(): Promise<void> {
    ...
    try {
      if (!fs.existsSync(this.chromaWorkingDir)) {
        ...
        console.log(
          `[CHROMA] Created working directory: ${this.chromaWorkingDir}`
        );
      }
    } catch (error) {
      throw new Error(
        `Failed to create directory ${this.chromaWorkingDir}: ${error}`
      );
    }
    ...
    try {
      ...
      if (existingProcesses.length > 0) {
        console.log(
          `[CHROMA] Found existing chroma_starter.exe process(es): ${existingProcesses
            .map((p) => p.pid)
            .join(", ")}`
        );
        ...
        console.log(
          "[CHROMA] No existing chroma_starter.exe process found, starting new one..."
        );
   ...
   */
  private async findChromaProcesses(): Promise<
          ...
          for (const line of lines) {
            ...
            if (parts.length >= 2) {
              ...
              if (!isNaN(pid)) {
                processes.push({
                  pid,
                  command: parts[10] || "chroma_starter.exe",
                });
              }
            }
          }
  ...
  private async spawnChromaProcess(): Promise<void> {
    ...
    const chromaExePath = await ChromaManager.getChromaExePath(
      this.chromaWorkingDir
    );
    const chromaDataPath = path.join(this.chromaWorkingDir, "data");
    console.log(
      `[CHROMA] Executable command: ${chromaExePath} run --port 19801 --path ${chromaDataPath}`
    );
    const chromaProcess = spawn(
      chromaExePath,
      ["run", "--port", "19801", "--path", chromaDataPath],
      {
    ...
    chromaProcess.on("exit", (code, signal) => {
      console.log(
        `[CHROMA] Process exited with code ${code} and signal ${signal}`
      );
    });
    ...
    // Wait for connection based on vector database type
  ...
  private async waitForConnection(): Promise<void> {
    ...
    if (retries >= maxRetries) {
      throw new Error(
        "[CHROMA] Failed to establish connection after maximum retries"
      );
    }
  }
  ...
  private startHealthCheck(): void {
    this.healthCheckInterval = setInterval(async () => {
      ...
      if (!isAlive) {
        console.log(
          "[CHROMA] Health check failed - no chroma_starter.exe process found, starting new one..."
        );
        try {
          await this.spawnChromaProcess();
        ...
        console.log(
          "[CHROMA] Health check passed - chroma_starter.exe process is running"
        );
      }
   ...
   */
  public static async getChromaExePath(
    ...
    try {
      if (fs.existsSync(userContextExePath)) {
        console.log(
          `[CHROMA] Found chroma_starter.exe in user context: ${userContextExePath}`
        );
    ...
    console.log(
      `[CHROMA] chroma_starter.exe not found, downloading to user context directory...`
    );
   ...
   */
  private static async downloadChromaStarter(
    targetPath: string
  ): Promise<void> {
    ...
    try {
      ...
      console.log(
        `[CHROMA] Successfully downloaded chroma_starter.exe to: ${targetPath}`
      );
    } catch (error) {
      ...
      get(url, (response) => {
        if (response.statusCode === 302 || response.statusCode === 301) {
          ...
          if (newUrl) {
            ...
            ChromaManager.downloadFile(newUrl, targetPath)
              .then(resolve)
              .catch(reject);
            return;
          }
